...
public class SettingsModelGenerator : IIncrementalGenerator
{
    ...
    private static void GenerateOne(SourceProductionContext context, Candidate candidate)
    {
        ...
        foreach (var field in symbol.GetMembers().OfType<IFieldSymbol>())
        {
            ...
            fields.Add(new FieldModel(field.Name, propertyName.EscapeIdentifier(), field.Type.ToGlobalTypeName(), kind));
        }
    ...
    private static string BuildSource(INamedTypeSymbol type, IReadOnlyList<FieldModel> fields, bool needsConstructor)
    {
        ...
        using (sb.Block($"{accessibility} partial class {type.Name.EscapeIdentifier()} : global::System.ComponentModel.INotifyPropertyChanged"))
        {
            ...
            if (needsConstructor)
            {
                using (sb.Block($"public {type.Name}()"))
                {
            ...
            foreach (var f in fields)
            {
                switch (f.Kind)
                {
                    case FieldKind.Scalar:
                        using (sb.Block($"public {f.TypeName} {f.PropertyName}"))
                        {
                            sb.AppendLine($"get => {f.FieldName};");
                            using (sb.Block("set"))
                            {
                                using (sb.Block($"if (!global::System.Collections.Generic.EqualityComparer<{f.TypeName}>.Default.Equals({f.FieldName}, value))"))
                                {
