...
public class SettingsDraftVmGenerator : IIncrementalGenerator
{
    ...
    private static void GenerateOne(SourceProductionContext context, Candidate candidate)
    {
        ...
        context.AddSource(
            ...
            SourceText.From(BuildSource(symbol, candidate.DocumentType.ToGlobalTypeName(), candidate.DraftRootPath, validProxies), Encoding.UTF8));
    }
        ...
        List<ProxyInfo> proxies)
    {
        ...
        sb.AppendLine("{");
        sb.AppendLine($"    private global::ComposableSettings.Document.SettingsEditingSession<{documentType}> _draftSession = null!;");
        sb.AppendLine($"    private global::ComposableSettings.Document.ISettingsDocumentStore<{documentType}> _draftStore = null!;");
        sb.AppendLine("    private bool _draftVmDisposed;");
        sb.AppendLine();
        ...
        sb.AppendLine("    }");
        sb.AppendLine();
        sb.AppendLine($"{accessibility} void DisposeGeneratedSettingsDraft()");
        sb.AppendLine("    {");
        sb.AppendLine("        if (_draftVmDisposed)");
        sb.AppendLine("            return;");
        sb.AppendLine("        _draftVmDisposed = true;");
        sb.AppendLine("        _draftStore.EffectiveChanged -= OnStoreEffectiveChanged;");
        sb.AppendLine("    }");
        sb.AppendLine();
        sb.AppendLine("    private void OnStoreEffectiveChanged(object? sender, global::System.EventArgs e)");
        sb.AppendLine("    {");
        sb.AppendLine("        if (_draftVmDisposed)");
        sb.AppendLine("            return;");
        ...
        sb.AppendLine("    }");
        sb.AppendLine();
        sb.AppendLine("    private void RefreshAllDraftProxies()");
        sb.AppendLine("    {");
        sb.AppendLine("        if (_draftVmDisposed)");
        sb.AppendLine("            return;");
        ...
        sb.AppendLine("    }");
        sb.AppendLine();
        sb.AppendLine("    private void OnDraftPropertyChanged(string memberPath)");
        sb.AppendLine("    {");
        sb.AppendLine("        if (_draftVmDisposed)");
        sb.AppendLine("            return;");
        ...
        sb.AppendLine("    }");
        sb.AppendLine();
        sb.AppendLine("    /// <summary>Optional hook after draft preview (e.g. derived labels).</summary>");
    ...
    private static void EmitProxyProperty(StringBuilder sb, ProxyInfo proxy)
    {
